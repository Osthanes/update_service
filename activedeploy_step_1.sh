...
if [[ -n "${original_grp}" ]]; then
  ...
  if [[ ${env_check} -eq '0' ]]; then
    ...
    if [ $? -eq 0 ]; then
      ...
      curl -s -X PUT --data "{\"organization_guid\": \"$CF_ORGANIZATION_ID\"}" -H "Authorization: ${TOOLCHAIN_TOKEN}" -H "Content-Type: application/json" "$AD_API_URL/v1/service_instances/$SERVICE_ID" > curlRes.json
